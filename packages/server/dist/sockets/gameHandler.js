        ...
        if (room && data.playlistUrl) {
            ...
            try {
                ...
                if (source === 'youtube') {
                    ...
                    rawTracks = await (0, youtube_1.getYouTubePlaylistTracks)(listId);
                }
                else {
                    // Pobierz Spotify ID
                    ...
                    rawTracks = await (0, spotify_1.getPlaylistTracks)(playlistId);
                }
                // Konwertuj na format Gry (konwertujemy całą dostępną pulę ze Scrapera, do ~100)
                const validTracks = rawTracks.map(st => ({
    ...
    socket.on('submitAnswer', (data) => {
        ...
        if (room && room.status === 'playing' && room.currentSegment && room.segmentResponses) {
            ...
            for (let s = 1; s < room.currentSegment; s++) {
                ...
                if (prevRes?.isAuthorCorrect)
                    alreadyGuessedAuthor = true;
                if (prevRes?.isTitleCorrect)
                    alreadyGuessedTitle = true;
            }
            ...
            if (playersAnswered >= room.players.length) {
                ...
                for (const p of room.players) {
                    ...
                    let hasTit = false;
                    for (let s = 1; s <= room.currentSegment; s++) {
                        if (room.segmentResponses[s]?.[p.id]?.isAuthorCorrect)
                            hasAuth = true;
                        if (room.segmentResponses[s]?.[p.id]?.isTitleCorrect)
                            hasTit = true;
                    }
                    if (hasAuth && hasTit)
                        playersCompleted++;
                            ...
                            if (currentRoom.currentSegment && currentRoom.currentSegment < 6) {
                                ...
                                for (const p of currentRoom.players) {
                                    ...
                                    for (let s = 1; s < currentRoom.currentSegment; s++) {
                                        if (currentRoom.segmentResponses[s]?.[p.id]?.isAuthorCorrect)
                                            hasAuth = true;
                                        if (currentRoom.segmentResponses[s]?.[p.id]?.isTitleCorrect)
                                            hasTit = true;
                                    }
                                    if (hasAuth && hasTit) {
